...
-- Damage types.
MikCEH.DAMAGETYPE_PHYSICAL = 0;
MikCEH.DAMAGETYPE_HOLY = 1;
MikCEH.DAMAGETYPE_FIRE = 2;
MikCEH.DAMAGETYPE_NATURE = 3;
MikCEH.DAMAGETYPE_FROST = 4;
MikCEH.DAMAGETYPE_SHADOW = 5;
MikCEH.DAMAGETYPE_ARCANE = 6;
...
function MikCEH.OnLoad()
	...
	if className == "DRUID" or className == "SHAMAN" or className == "PRIEST" then
	...
	if GetNampowerVersion then
		table.insert(listenEvents, "SPELL_DAMAGE_EVENT_SELF");    -- Outgoing DoTs
	else
...
function MikCEH.OnEvent()
	...
	elseif (event == "SPELL_DAMAGE_EVENT_SELF") then
		-- https://github.com/pepopo978/nampower?tab=readme-ov-file#spell_damage_event_self-and-spell_damage_event_other
		local targetGuidStr, casterGuidStr, spellId, amount, mitigationStr, hitInfo, spellSchool, effectAuraStr = arg1, arg2, arg3, arg4, arg5, arg6, arg7, arg8;
		local spellName, _ = GetSpellNameAndRankForId(spellId)
		...
		if amount > 0 then
			local eventData = MikCEH.GetDamageEventData(
					MikCEH.DIRECTIONTYPE_PLAYER_OUTGOING,
					MikCEH.ACTIONTYPE_HIT,
					MikCEH.HITTYPE_OVER_TIME,
					tonumber(spellSchool),
					amount,
					spellName,
					playerName
			);
...
function MikCEH.ParseForIncomingSpellHeals(combatMessage)
	...
	if (capturedData ~= nil) then
		...
		if GetLocale() ~= "zhCN" then
			-- edge case for chinese client (unable to show overheal amount for hot casted on self by other)
			MikCEH.PopulateOverhealData(eventData); -- athenne add
		end
		eventData.Name = capturedData.Name
	...
	if (capturedData ~= nil and UnitName("pet") and string.find(combatMessage, UnitName("pet"))) then
		...
		eventData.Name = UnitName("pet")
		if GetLocale() ~= "zhCN" then
			-- edge case for chinese client (unable to show overheal amount for hot casted on your pet by other)
			MikCEH.PopulateOverhealData(eventData); -- athenne add
		end
		eventData.Name = capturedData.Name
		...
		for index = 0, stringLength do
			...
			if (formatCode == nil) then
				...
				elseif (string.find(currentChar, "[cEefgGiouXxqs]")) then
					...
					if GetLocale() == "zhCN" and globalStringName == "HEALEDSELFOTHER" then
						-- edge for outgoing heals on others case for zhCN client
...
function MikCEH.GetUnorderedCaptureDataTable(c1, c2, c3, c4, c5, c6, c7, c8, c9)
	...
	if (c1 ~= nil) then
		table.insert(unorderedCaptureData, c1);
	end
	if (c2 ~= nil) then
		table.insert(unorderedCaptureData, c2);
	end
	if (c3 ~= nil) then
		table.insert(unorderedCaptureData, c3);
	end
	if (c4 ~= nil) then
		table.insert(unorderedCaptureData, c4);
	end
	if (c5 ~= nil) then
		table.insert(unorderedCaptureData, c5);
	end
	if (c6 ~= nil) then
		table.insert(unorderedCaptureData, c6);
	end
	if (c7 ~= nil) then
		table.insert(unorderedCaptureData, c7);
	end
	if (c8 ~= nil) then
		table.insert(unorderedCaptureData, c8);
	end
	if (c9 ~= nil) then
		table.insert(unorderedCaptureData, c9);
	end
