...
-- add known spells to cache
--for true do
--	local spellName=GetSpellName(i, "spell"); print(tostring(i) .. spellName)
--	break
--end
...
function MikSBT.CombatEventsHandler(combatEvent)
	...
	elseif (combatEvent.PartialActionType == MikCEH.PARTIALACTIONTYPE_VULNERABLE) then
		if (MikSBT.MSG_VULNERABLE_TRAILER) then
			animationEvent.PartialEffectText = string.gsub(MikSBT.MSG_VULNERABLE_TRAILER, "%%d", combatEvent.PartialAmount);
		end
...
function MikSBT.UpdateProfiles()
	...
	for _, profile in MikSBT_Save.Profiles do
		...
		if (profile.CreationVersion < 4.1) then
			...
			if profile.Triggers.MSBT_TRIGGER_HAND_OF_JUSTICE then
				-- Delete Hand of Justice Trigger
...
function MikSBT.AddAnimation(animationEvent)
	...
	local texture
	if animationEvent.Texture then
		texture = animationEvent.Texture
	end
	if animationEvent.EffectName then
		...
		elseif not texture and SpellInfo then
			_, _, texture = SpellInfo(name)
			if texture then
				ICON_CACHE[strlower(name)] = texture
			end
		end
